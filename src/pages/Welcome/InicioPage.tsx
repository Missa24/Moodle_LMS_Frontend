        ...
        useMemo(
            () =>
                categoriaIdSeleccionada
                    ? categorias.find((categoria,) => categoria.id === categoriaIdSeleccionada,) ?? null
                    : null,
            [categoriaIdSeleccionada, categorias,],
        );

    const { data: subcategorias = [], } = useSubcategoriasCursos(
        categoriaIdSeleccionada ?? "",
    );

    const { data, isLoading, isError, error, } =
        useCursos(1, 12, search, categoriaActiva,);
                ...
                <div className="min-w-0 space-y-4">
                    <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div className="min-w-0">
                            <AppTitle
                                title={categoriaSeleccionada ? categoriaSeleccionada.nombre : "Cursos disponibles"}
                                subtitle={
                                    totalCursos !==
                                        undefined
