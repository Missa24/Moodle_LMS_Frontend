...
import { ModuleListCardSkeleton } from "./ModuleListCardSkeleton";

import { useCursoPorSlug } from "./service";
import { useModulos } from "../modulos/service";

export default function CursoPageContent() {
    const { slug } = useParams<{ slug: string }>();

    const { data: curso } = useCursoPorSlug(slug);
    ...
    return (
        <>
            <SEO
                title={curso?.nombre ?? "Módulos del curso"}
                description={descripcionCurso}
                url={`/cursos/${slug ?? ""}`}
                image={curso?.rutaPortada ?? undefined}
                        ...
                        {isLoading ? (
                            <div className="space-y-3 sm:space-y-4">
                                {Array.from({ length: 5 }).map((_, index) => (
                                    <ModuleListCardSkeleton key={index} />
                                ))}
                            </div>
                        ) : isError ? (
